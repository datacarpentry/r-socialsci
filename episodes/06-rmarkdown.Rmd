...
asterisks and underscores, **_really_**, `**_really_**. Just be consistent...
...
```
Then we can create a list for the variables using `-`, `+`, or `*`.
...
Now we can render the document into HTML by clicking the **Knit** button in the top
...
```{r load-tidyverse, echo=FALSE, message=FALSE, purl=FALSE}
library(tidyverse)
interviews <- read_csv("data/SAFI_clean.csv", na = "NULL")
```

To load **tidyverse** and our `SAFI_clean.csv` file, we will insert a chunk and 
call it 'setup'. Since we don't want to show the chunk or the output of the chunk 
in our knitted HTML document, we use `{r setup, include=FALSE}`. 
...
> relative to the .Rmd document and not the project root. 
>
> Use the `here()` function to keep the file paths consistent within your project.
{: .callout}
...
Or, you can come up with something more creative
...
to use the `kable` function from the **knitr** package.
...
in the output document. The options are entered in the code chunk after `chunk-name`
and separated by commas, e.g. `{r chunk-name, eval=FALSE, echo=TRUE}`.
...
| `warning=<TRUE,FALSE>` | Whether or not you want your output document to display potential warning messages produced by your code. |
| `message=<TRUE,FALSE>` | Whether or not you want your output document to display potential messages produced by your code. |
...
including `r` after the first backtick. Each time you **Knit** your document, 
the code will be executed and only the output from the executed code will be shown
in the output document. If, for whatever reason, our data and/or code has changed, 
the document will automatically update the output of our code accordingly. Pretty
useful for those of us who are prone to data entry errors!
...
<pre>
&#96;&#96;&#96;{r chunk-name, fig.cap="I made this plot while attending an awesome Data Carpentries workshop where I learned a ton of cool stuff!"}
Code for plot
&#96;&#96;&#96;
</pre>
