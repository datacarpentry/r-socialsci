...
You can use the `.` caharacter as part of the name but this is not recommended. If you wish make longer variable names more readable, using the `_` character helps. 
...
The sqrt function only takes a single parameter. It doesn't have to be a literal value like `4` it could be a variable name or
even an expression.
