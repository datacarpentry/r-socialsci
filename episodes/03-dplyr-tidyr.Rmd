...
select(interviews, village, village:respondent_wall_type)
```
...
dataframe to adhere to (e.g. village name is God). We can chain a series of 
...
2. a list of *cols* (columns) that are to be reshaped; these can be specified 
   ...
   or with a `vars()` or vector (`c()`) command if the columns are spread
...
interviews_long <- interviews_wide %>%
    pivot_longer(cols = vars(burntbricks:sunbricks),
