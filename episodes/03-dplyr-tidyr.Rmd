...
For each row, only one of the newly created wall type columns will be `TRUE`. 
The default fill for these is `NA`, but we want them to be `FALSE`. We will use 
...
interviews_plotting <- interviews %>%
    ## pivot wider data by items_owned
    separate_rows(items_owned, sep=";") %>%
    mutate(items_owned_logical = TRUE) %>%
    pivot_wider(names_from = items_owned, 
                values_from = items_owned_logical, 
