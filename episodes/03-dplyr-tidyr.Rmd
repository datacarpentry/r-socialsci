...
For each row in our newly pivoted table, only one of the newly created wall type columns will be `TRUE`. 
The default value that `pivot_wider` uses to fill the other wall types is `NA`, but we want them to be `FALSE`. We will use `values_fill = list(wall_type_logcial = FALSE)` in `pivot_wider()` to fill the rest of the new columns for that row with `FALSE`.
