...
When we group on two variables, such as village and wall type, and summarize with one statistic, for example the number of observations, we get a data frame with three columns:
...
The first column denotes the village, the second column denotes the wall type, and the third column the number of households in this village that have this wallt ype
To decide whether `interviews_grouped` adheres to the three rules for a tidy dataset, we need to consider what we define to be our variables, and what to be the observations.
If we want to compare different wall types in the different villages, we can create a new dataset where we have one row for each village and one column for each wall type, with values denoting the number of households. This is a "wide" data format, as opposed to the "long" format that results from the group_by - summarize pipeline.
...
Let's use `pivot_wider()` to transformed our grouped data to a wide format:
```{r, purl = FALSE}
walltypes_per_village_wide <- interviews %>%
  group_by(village, respondent_wall_type) %>%
  summarize(no_housholds = n()) %>%
  pivot_wider(names_from = respondent_wall_type,
              values_from = no_housholds,
              values_fill = 0)
walltypes_per_village_wide
```
View `walltypes_per_village_wide`, and notice how the original *values* of the `respondent_wall_type` column now became *column names*. This allows for easy comparison between the wall types.


> ## Exercise
>
> Create a new dataframe (named `interviews_per_day`) that has one row for each interview date
> and one column for each village. The values denote the number of interviews conducted that day.
>
> > ## Solution
> >
> > ```{r}
> > interviews_per_day_wide <- interviews %>%
...
of `walltypes_per_village_wide`, where the building materials are column names, but we
...
1. the data - `walltypes_per_village_wide`
...
4. the *values_to* column will be a character string of the name of the 
   ...
   ("no_interviews"). This column will be populated with the counts of all the columns in *cols*


```{r, purl = FALSE}
walltypes_per_village_long <- walltypes_per_village_wide %>%
...
### Use pivot_wider() to transform interviews
In the previous example, we saw how we can use `pivot_wider()` on grouped and summarized data, that has an obvious long format, but we can also use it to transform the `interviews` table to different formats. In `interviews`, each row contains the values of variables associated with each
...
To facilitate this comparison we would need to create a new table that has the same number of rows as our original table, namely one per household. However, instead of one variable `respondent_wall_type` denoting the wall construction materials, we want the values of `respondent_wall_type` (e.g. muddaub,
burntbricks, cement, sunbricks) to become the names of column variables and
