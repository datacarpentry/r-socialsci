...
manipulation tasks. It is built to work directly with dataframes, with many
...
**`tidyr`** gives you tools for this and more sophisticated  data manipulation.
...
dataframe, use `select()`. The first argument to this function is the data
...
for rows with `village == "God"`, *then* we `select` columns `no_membrs` and
`years_liv`. The **`dplyr`** functions by themselves are somewhat simple,
but by combining them into linear workflows with the pipe, we can accomplish
more complex manipulations of dataframes.
...
to structure our data leading to the three rules defining a "tidy" dataset:

1. Each variable has its own column
2. Each observation has its own row
3. Each value must have its own cell

<!-- I'm not sure how I feel about this rule, I think that you can break down  -->
<!-- long and wide data in terms of rules 1 and 2.  -->
...
with each record collected (each interview in the villages), where the unique
identifier for each row is the `key_ID`. Without the `key_ID` it would be 
difficult to tell observations apart, since we have multiple observations 
for each village, date combination (as seen below). 
...
We also notice that no `key_ID` was surveyed more than once. As seen in the code 
below, for each interview date in each village no `key_ID`s are the same. Thus, 
...
2. a list of *cols* (columns) that are to be reshaped; these can be specified 
   ...
   or with a vector (`c()`) if the columns are spread throughout the dataframe. 
...
list. We can use the `rename()` function to change the name of this column to
...
Or calculate the average number of items from the list owned by respondents in
each village:
