...
For each row, there will be newly created wall type columns that are not `TRUE`. The
default fill would be `NA`, but we want them to be `FALSE`. We will use 
`values_fill = list(wall_type_logcial = FALSE)` in `pivot_wider()` to fill the 
rest of the new columns for that row with `FALSE`.
...
2. the *cols* are the names of the columns we will use to fill the key variable are
...
value of the "respondent_wall_type" and "wall_type_logical" columns. View the data to see what this looks like.
...
structure. Notice that the rows have been reordered in `interviews_long` such
